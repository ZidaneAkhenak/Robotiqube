...
import numpy as np
import solver as sv
from execute_solution import execute_solution  # 🔥 AJOUTÉ
...
detected_faces = []  # 6 faces, 4 couleurs chacune
current_face = 0

# Positions des 4 carrés au centre pour capture webcam
...
def guess_color_from_hsv(hsv_pixel):
    h, s, v = hsv_pixel
    if v < 50:
        return 'B'  # Probably blue or dark face
    if s < 50:
        return 'U'  # Probably white
    if h < 10 or h > 160:
        return 'R'
    elif 10 < h < 25:
        return 'L'
...
def modify_colors(facelets):
    selected = 0  # index du carré sélectionné
    positions = [(50, 50), (150, 50), (50, 150), (150, 150)]  # coordonnées des 4 carrés
    ...
    while True:
        ...
        for i, facelet in enumerate(facelets):
            ...
            if i == selected:
                cv2.rectangle(img, (x, y), (x + 80, y + 80), (0, 255, 255), 2)  # sélection jaune
...
def faces_to_string(faces):
    """
    Convertit la liste des couleurs des faces en une chaîne de caractères.
    Chaque face est représentée par une séquence de 4 couleurs (de haut en bas, de gauche à droite).
    """
...
def main():
    ...
    for i in range(6):
        ...
        # Modification des couleurs immédiatement après chaque capture
    ...
    # 🔥 Appel à l’envoi des mouvements au robot
    execute_solution(cube_string)  # 🔥 AJOUTÉ
